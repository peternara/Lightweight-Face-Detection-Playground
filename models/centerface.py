...
class CenterFace(nn.Module):
    ...
    def __init__(self, phase, cfg):
        ...
        self.wh_head, self.cls_head, self.ctr_head = self.loss_head()
    ...
    def loss_head(self):
        ...
        ctr_heads += [InvertedResidual_Head(inp=32, oup=2, kernel=3, stride=1, expand_ratio=1)]
        ctr_heads += [InvertedResidual_Head(inp=32, oup=2, kernel=3, stride=1, expand_ratio=1)]
        ctr_heads += [InvertedResidual_Head(inp=32, oup=2, kernel=3, stride=1, expand_ratio=1)]

        wh_heads += [InvertedResidual_Head(inp=32, oup=2, kernel=3, stride=1, expand_ratio=1)]
        wh_heads += [InvertedResidual_Head(inp=32, oup=2, kernel=3, stride=1, expand_ratio=1)]
        wh_heads += [InvertedResidual_Head(inp=32, oup=2, kernel=3, stride=1, expand_ratio=1)]
        ...
        return nn.Sequential(*wh_heads), nn.Sequential(*cls_heads), nn.Sequential(*ctr_heads)
    ...
    def forward(self, x):
        ...
        if self.use_ldmk:
            ldmk = self.ldmk_head(p3)

        for idx, t in enumerate(features):
            # print(idx)
            #bchw -> bhwc
            wh.append(self.wh_head[idx](t).permute(0, 2, 3, 1).contiguous())
            cls.append(self.cls_head[idx](t).permute(0, 2, 3, 1).contiguous())
            ctr.append(self.ctr_head[idx](t).permute(0, 2, 3, 1).contiguous())

        wh = torch.cat([o.view(o.size(0), -1) for o in wh], 1)
        cls = torch.cat([o.view(o.size(0), -1) for o in cls], 1)
        ctr = torch.cat([o.view(o.size(0), -1) for o in ctr], 1)

        # if self.phase == "test":
        #     wh = wh.exp()

        if self.use_ldmk:
            output = (wh.view(batchsize, -1, 2),
                      cls.sigmoid().view(batchsize, -1, 1),
                      ctr.view(batchsize, -1, 2),
                      ldmk.view(batchsize, -1, 10))
        else:
            output = (wh.view(batchsize, -1, 2),
                      cls.sigmoid().view(batchsize, -1, 1),
                      ctr.view(batchsize, -1, 2))
    ...
    def post_process(self,
        ...
        boxes = self.decode_box(wh.exp(), box_cords.unsqueeze(0)).squeeze(0)
        boxes = boxes / resize
    ...
    def decode_box(self, box_pred, box_cord):
...
def draw_bboxes(img_origin, bboxes_scores, img_save_name):
    """
...
if __name__=="__main__":
    ...
    from utils.misc.checkpoint import *
    import torch.backends.cudnn as cudnn
    from utils.misc import add_flops_counting_methods, flops_to_string, get_model_parameters_number

    img_name = "./images/1_Handshaking_Handshaking_1_579.jpg"
    trained_model = "./checkponts/CenterFace.pth"
    use_cuda = True
    img_origin = np.float32(cv2.imread(img_name, cv2.IMREAD_COLOR))
    img_save_name = "1_Handshaking_Handshaking_1_579_bbox.jpg"
    net_cfg = {
        'net_name': 'CenterFace',
        'num_classes': 1,
        'use_ldmk': False
    ...
    # foward
    with torch.no_grad():
        wh, conf, _ = net(img)  # forward pass
        dets = net.post_process(wh,
                                ...
                                resize)
    # draw results
    draw_bboxes(img_origin, dets, img_save_name)